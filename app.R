...
embedded_iteration_3_code <- c(
  ...
  "}",
  "",
  "# \"n_totaal_gebruikers\" is the \"Statistic type\" for a raw headcount (number",
  "# of users), not a monetary amount -- it should never be CPI-adjusted and",
  "# should always be displayed as a whole number, regardless of whether the",
  "# selected outcome variable is otherwise a cost/income field.",
  "is_count_type <- function(type_value) {",
  "  identical(type_value, \"n_totaal_gebruikers\")",
  "}",
  "",
  ...
  "}",
  "",
  "format_graph_number <- function(x, cost = FALSE, digits = NULL, axis = FALSE) {",
  "  values <- suppressWarnings(as.numeric(x))",
  "  # Costs/income round to whole numbers (0 decimals); everything else",
  "  # (e.g. binary/proportion outcomes) keeps 3 decimals, unless the caller",
  "  # passes an explicit digits override (e.g. for raw headcounts).",
  "  if (is.null(digits)) digits <- if (isTRUE(cost)) 0 else 3",
  ...
  "",
  "    # Built directly with plotly (not ggplot -> ggplotly) for two reasons.",
  "    # First, ggplotly's facet-width calculation for facet_wrap is",
  "    # unreliable, so each diagnosis-group panel is its own subplot with an",
  "    # explicit, guaranteed-equal width. Second, ggplotly converts geom_area",
  "    # into a filled trace whose hover only fires on the fill as a whole,",
  "    # always showing the first point regardless of mouse x-position.",
  "    # Building the stacked area manually with add_trace and",
  "    # hoveron = \"points\" ties hover text to the actual nearest year.",
  ...
  "",
  "      df_stack <- df_panel |>",
  "        dplyr::arrange(name_factor, year) |>",
  "        dplyr::group_by(year) |>",
  "        dplyr::mutate(stack_top = cumsum(tidyr::replace_na(value_mln, 0))) |>",
  "        dplyr::ungroup()",
  "",
  "      gp <- plotly::plot_ly()",
  "      for (gname in fill_values) {",
  "        sub <- df_stack[as.character(df_stack$name_factor) == gname, , drop = FALSE]",
  ...
  "          type = \"scatter\",",
  "          mode = \"lines\",",
  "          line = list(width = 0.5, color = fill_palette[[gname]]),",
  "          fill = \"tonexty\",",
  "          fillcolor = grDevices::adjustcolor(fill_palette[[gname]], alpha.f = 0.85),",
  "          name = fill_labels[[gname]],",
  "          legendgroup = gname,",
  "          showlegend = (i == 1),",
  "          text = ~tooltip,",
  "          hoverinfo = \"text\",",
  ...
  "      plotly::layout(",
  "        gp,",
  "        title = list(text = lvl, font = list(size = 12)),",
  "        xaxis = list(title = \"Year\", tickmode = \"linear\", dtick = 1, tickangle = -45),",
  "        yaxis = list(title = if (i == 1) \"Mln EUR\" else \"\", tickformat = \",.0f\", rangemode = \"tozero\")",
  "      )",
  "    })",
  "",
  "    plotly::subplot(",
  ...
  "        !isTRUE(is_monetary_variable(selected_names[[1]])) ||",
  "        is_count_type(input$dmean_type)) {",
  "      return(NULL)",
  "    }",
  ...
  "    count_plot <- is_count_type(input$dmean_type)",
  "    cost_plot <- length(selected_names) == 1 && is_monetary_variable(selected_names[[1]]) && !count_plot",
  "",
  ...
  "        # it recomputes the year-specific nominal figure. This CPI math only",
  "        # ever applies to monetary variables -- non-monetary outcomes (binary",
  "        # flags, headcounts like n_totaal_gebruikers, etc.) always show the",
  "        # raw stored value, never multiplied by a price index.",
  "        value_adjusted_2023 = value,",
  "        value_nominal = dplyr::if_else(!is.na(cpi) & cpi > 0, value * cpi, value),",
  "        value_plot = if (!cost_plot) {",
  "          value_adjusted_2023",
  "        } else if (is.null(input$dmean_adjust_inflation) || isTRUE(input$dmean_adjust_inflation)) {",
  "          value_adjusted_2023",
  "        } else {",
  "          value_nominal",
  "        },",
  "        value_label = format_graph_number(value_plot, cost = cost_plot, digits = if (count_plot) 0 else NULL),",
  ...
  "      scale_linetype_manual(values = hiv_linetype_values) +",
  "      scale_x_continuous(breaks = sort(unique(df$year))) +",
  "      scale_y_continuous(labels = function(x) format_graph_number(x, cost = cost_plot, axis = TRUE, digits = if (count_plot) 0 else NULL)) +",
  "      theme_classic(base_size = 13) +",
  "      theme(legend.position = \"bottom\", axis.text.x = element_text(angle = 45, hjust = 1)) +",
  "      labs(",
  ...
  "    count_plot <- is_count_type(input$emean_type)",
  "    cost_plot <- length(selected_names) == 1 && is_monetary_variable(selected_names[[1]]) && !count_plot",
  "",
  "    df <- df |>",
  ...
  "    df |>",
  "      dplyr::mutate(",
  "        value_label = format_graph_number(value, cost = cost_plot, digits = if (count_plot) 0 else NULL),",
  "        value_plot = value,",
  "        series_label = if (group_var != \"none\") {",
  "          paste0(group_value, \" - \", has_hiv_label)",
  "        } else {",
